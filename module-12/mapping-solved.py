...
class QuadMap:
	...
	def _get_child_containing_point(self, node, point):
		...
		while depth < self.max_depth:
			if len(current_node.children) == 0:
				if current_node.state == state:
					return # Already has the state, no need to split or update
				current_node.split()
			
			current_node = self._get_child_containing_point(current_node, point)
			depth += 1
	...
	def ray_update(self, origin: np.ndarray, endpoint: np.ndarray):
		...
		"""
		# Calculate step size based on max resolution
		min_cell_size = self.size / (2**self.max_depth)
		step_size = min_cell_size / 2.0
		...
		direction = direction / distance # Normalized
		
		current_dist = 0.0
		while current_dist < distance - step_size:
	...
	def to_occupancygrid(self):
		...
		"""
		grid_res = 2**self.max_depth
		total_cells = grid_res * grid_res
		data = [-1] * total_cells
		
		cell_size = self.size / grid_res
		start_x = self.origin[0] - self.size / 2.0 + cell_size / 2.0
		start_y = self.origin[1] - self.size / 2.0 + cell_size / 2.0
		
		for i in range(grid_res): # Row (y)
			for j in range(grid_res): # Col (x)
				x = start_x + j * cell_size
				y = start_y + i * cell_size
				state = self.get_state(np.array([x, y]))
				
				ros_state = -1
				if state == MapType.UNOCCUPIED:
					ros_state = 0
				elif state == MapType.OCCUPIED:
					ros_state = 100
				
				data[i * grid_res + j] = ros_state
...
class QuadMapNode:
	...
	def split(self):
		...
		hs = self.size / 4.0 # Half of child size, quarter of parent size
		
		# Children origins
		nw_origin = self.origin + np.array([-hs, hs])
		ne_origin = self.origin + np.array([hs, hs])
		sw_origin = self.origin + np.array([-hs, -hs])
		se_origin = self.origin + np.array([hs, -hs])
		
		self.children = [
			QuadMapNode(parent=self, origin=nw_origin),
			QuadMapNode(parent=self, origin=ne_origin),
			QuadMapNode(parent=self, origin=sw_origin),
			QuadMapNode(parent=self, origin=se_origin)
		]
		
		# Children inherit the parent's state initially
	...
	def combine(self):
		...
		# Check if all children are leaves and have the same state
		first_state = self.children[0].state
		for child in self.children:
			if len(child.children) > 0 or child.state != first_state:
				return # Cannot combine
		
		# All children have the same state and no children of their own
